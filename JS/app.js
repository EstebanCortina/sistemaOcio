...
$(function () {
  ...
  $(document).on('click', '.resultados', function () {
    ...
    let tameplate = `
    <div codigo=${id} nombre>
      <p>Nombre: ${nombre} Calle: ${calle} Ciudad: ${ciudad} Información al correo: ${email} Telefono: ${telefono}</p>
      <p>¿Qué encontrarás aquí?
      <br>
      "${des}"
      </p>
      <button class='btnVerSalas'>Ver actividades</button>
      </div>
    `;
    $('#info').html(tameplate);
...
*/
  ...
  $(document).on('click', '.btnVerSalas', function () {
    ...
    $.post('../PHP/salasInfo.php', { id }, function (response) {
      ...
      information.forEach(data => {
        template += `
        <div codigo='${data.codigo}' class='${data.salaNombre}'>
